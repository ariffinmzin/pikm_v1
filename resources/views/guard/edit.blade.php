<x-layouts.app>
    ...
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-lg">
        <form action="{{ route('guard.update', $guard) }}" method="POST" enctype="multipart/form-data"
            class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <div
                            class="px-4 py-2 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 font-mono">
                            ●●●●●●-●●-{{ $guard->nric_last4 }}
                        </div>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                            NRIC cannot be changed after creation
                        </p>
                    </div>

                    <div>
                        <x-forms.input label="Full Name" name="full_name" required
                            placeholder="Enter full name as per NRIC" :value="old('full_name', $guard->full_name)" />
                    </div>

                    <div>
                        <x-forms.input label="Date of Birth" name="dob" type="date" :value="old('dob', $guard->dob?->format('Y-m-d'))" />
                    </div>
                    ...
                    <div>
                        ...
                        <select name="gender" id="gender"
                            ...
                            <option value="M" {{ old('gender', $guard->gender) == 'M' ? 'selected' : '' }}>Male
                            </option>
                            <option value="F" {{ old('gender', $guard->gender) == 'F' ? 'selected' : '' }}>Female
                            </option>
                        </select>
                        @error('gender')
                    ...
                    <div>
                        ...
                        <select name="blood_type" id="blood_type"
                            ...
                            @foreach (['A+', 'A-', 'B+', 'B-', 'O+', 'O-', 'AB+', 'AB-'] as $type)
                                <option value="{{ $type }}"
                                    {{ old('blood_type', $guard->blood_type) == $type ? 'selected' : '' }}>
                                    {{ $type }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="space-y-6">
                    <div>
                        <x-forms.input label="Contact Number" name="contact_no" placeholder="Enter contact number"
                            :value="old('contact_no', $guard->contact_no)" />
                    </div>

                    <div>
                        <x-forms.input label="Email Address" name="email" type="email"
                            placeholder="Enter email address" :value="old('email', $guard->email)" />
                    </div>
                    ...
                    <div>
                        ...
                        @if ($guard->photo_path)
                            <div class="mb-2">
                                <img src="{{ Storage::url($guard->photo_path) }}" alt="Guard photo"
                        ...
                        @enderror
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                            Leave empty to keep the current photo. Maximum size 2MB.
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="remarks" id="remarks" rows="3"
                            class="w-full px-4 py-2 rounded-lg text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                            placeholder="Enter any additional remarks">{{ old('remarks', $guard->remarks) }}</textarea>
                        @error('remarks')
                            <span class="text-red-500">{{ $message }}</span>
                        @enderror
                    </div>
